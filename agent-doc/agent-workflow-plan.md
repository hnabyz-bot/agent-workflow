...
> **⚠️ 개정 배경**: Copilot의 전문가 리뷰([copilot-opinion-agent-workflow.md](copilot-opinion-agent-workflow.md))를 반영하여, 실전 경험 기반으로 각 Agent의 역할을 재조정하고 검증 메커니즘을 추가했습니다.
...
2.  **Gemini (검토 및 개선 + 실행 검증)**:
    ...
    *   **작업**:
        ...
        *   **🆕 실행 가능성 검증**: 계획에 포함된 핵심 모듈이 실제로 구현 가능한지 코드 레벨에서 확인합니다.
...
1.  **Gemini (설계 주도 - 30%)**:
    ...
    *   **작업**:
        ...
        *   개발 중 발생하는 복잡한 오류나 예상치 못한 문제들을 **근본적으로 해결**합니다.
...
2.  **Copilot (구현 주도 - 40%)**:
    ...
    *   **작업**:
        ...
        *   **🆕 실시간 검증**: `get_errors` 도구로 각 파일 편집 후 즉시 오류를 확인하여 품질을 보장합니다.
...
1.  **Copilot (마무리 개발 - 40%)**:
    *   **역할**: UI 개발자, DevOps 엔지니어
    *   **작업**:
        *   개발자가 API 문서를 참고하여 프론트엔드 UI 컴포넌트를 만들거나, 간단한 설정 스크립트, 배포 스크립트 등을 작성할 때 **빠른 코드 생성**을 지원합니다.
        *   **🆕 추가**: CI/CD 파이프라인 스크립트, Docker 설정, 환경 설정 파일 등을 자동 생성합니다.
        *   최종 코드 품질 검증: `get_errors`로 전체 프로젝트의 오류를 최종 점검합니다.
...
3.  **Gemini (최종 검수 및 보고)**:
    ...
    *   **작업**:
        ...
        *   **🆕 추가**: 기술 부채(Technical Debt) 목록, 향후 개선 권장사항, 보안 점검 결과를 포함합니다.
