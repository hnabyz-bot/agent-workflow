...
**문제 인식:** 초기 설계에서 Claude가 Phase 1(계획), Phase 2(테스트 설계 + 문서 동기화), Phase 3(최종 문서화)에 모두 투입되어 과부하 발생 위험이 있었습니다.

**해결 원칙:**
1. **테스트 설계 통합:** Gemini가 "테스트 전략 + 테스트 케이스 설계"를 통합 수행 (Claude는 표현만 담당)
2. **문서 동기화 최적화:** 매 마일스톤마다 Claude 호출 대신, 2-3개 마일스톤 묶음 단위로 일괄 문서화
3. **Agent 투입 횟수 최소화:** 전환 오버헤드 감소 목적
...
| 작업 | 주관 | 상세 지침 (사용자 행동) |
| :--- | :--- | :--- |
| 1. 테스트 전략 및 설계 | **Gemini** | (마일스톤 시작 시) **To Gemini:** "이번 마일스톤의 기능은 [기능 요약]이야. 테스트 커버리지 목표(e.g., 80%)와 중점적으로 검증할 시나리오(e.g., 동시성, 예외처리)를 포함한 테스트 전략을 제안하고, 주요 테스트 케이스를 의사코드로 설계해줘." |
| 2. 테스트 표현 (선택) | **Claude** | (필요 시) **To Claude:** "첨부한 Gemini의 테스트 케이스 의사코드를 BDD(Given-When-Then) 스타일의 명세로 표현해줘." (복잡한 경우만) |
| 3. 설계/구현 | **Copilot** | **[핵심 작업] To Copilot:** "첨부한 설계와 테스트 케이스를 바탕으로 실제 코드를 구현하고, 모든 테스트를 통과시켜줘. 막히는 설계 문제가 발생하면 즉시 보고해." |
| 4. 동기화 | **Copilot** | (마일스톤 종료 시) `project_status.md`에 변경 요약(변경 파일 목록, 주요 기능 3줄 요약) 기록. **Claude 호출은 2-3개 마일스톤 묶음 또는 Phase 종료 시에만.** |
...
Phase 2: Gemini 테스트 설계 → Copilot 구현 (Claude는 Phase 종료 시만)
Phase 3: Copilot DevOps + Claude 문서 + Gemini 검수
```
**오버헤드**: 20% (개선: 기존 25% → 20%)
**효율 증가**: +40-50%
**권장**: 품질 게이트 엄격 적용, 2-3 마일스톤 묶음 문서화
...
## 8. 대규모 코드베이스 및 레거시 프로젝트 적용 전략

### 8.1 문제 정의
현재 워크플로우는 **신규 프로젝트(Green Field)**에 최적화되어 있습니다.
100k+ LOC 레거시 코드베이스 또는 기존 프로젝트 개선 시, 추가 전략이 필요합니다.

### 8.2 레거시 프로젝트 진입 전략

#### Phase 0.5: 코드베이스 이해 (신규 추가)
> **목표:** 기존 코드의 구조, 패턴, 기술 부채를 파악하여 안전한 개입 지점 찾기

| 작업 | 주관 | 상세 지침 |
| :--- | :--- | :--- |
| 1. 코드 구조 분석 | **Gemini** | **To Gemini:** "이 프로젝트의 주요 모듈 구조, 의존성 그래프, 핵심 데이터 플로우를 분석해줘. 특히 변경 시 파급 효과가 큰 'Critical Path' 3-5개를 식별해줘." |
| 2. 기술 부채 목록화 | **Gemini** | **To Gemini:** "코드 복잡도, 테스트 커버리지, 중복 코드를 분석하여 기술 부채 목록을 우선순위별로 작성해줘." |
| 3. 현황 문서화 | **Claude** | **To Claude:** "Gemini의 분석 결과를 바탕으로 '프로젝트 현황 보고서'를 작성해줘. (아키텍처 다이어그램, 기술 스택, 개선 우선순위 포함)" |

#### 부분 적용 전략 (Critical Path 우선)
```
1. Critical Path 식별 (Gemini 분석 기반)
2. 해당 영역만 워크플로우 적용 (나머지는 기존 방식 유지)
3. 2주 파일럿 → 효과 측정 (metrics_template.md 활용)
4. 성공 시 점진적 확장 (모듈 단위)
```

#### 점진적 확장 로드맵
```
Week 1-2:  Phase 0.5 (코드베이스 이해) + Critical Path 1개 개선
Week 3-4:  Critical Path 2-3개 추가 적용
Week 5-6:  효과 측정 및 팀 회고
Week 7+:   전체 모듈로 확장 또는 중단 결정
```

### 8.3 대규모 코드베이스 특수 규칙

1. **컨텍스트 분할:** 100k+ LOC는 한 번에 분석 불가
   - 모듈별로 `project_status.md` 분리 (예: `project_status_auth.md`, `project_status_payment.md`)
   - Agent에게 "현재 모듈만" 전달

2. **변경 범위 제한:** 한 마일스톤에서 수정 파일 10개 이하로 제한

3. **회귀 테스트 강화:** 레거시 코드 수정 시 기존 테스트 전체 실행 필수
...
4. ✅ **품질 게이트 준수** (단계별 검증)
5. ✅ **20-25% 오버헤드 수용** (품질 향상으로 상쇄)
6. ✅ **metrics_template.md로 효과 측정** (가설 검증)
...
5. ❌ **레거시 코드에 무계획 진입 금지** (Phase 0.5 필수)
